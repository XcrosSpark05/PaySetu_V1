...
class SmsReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if (intent.action == Telephony.Sms.Intents.SMS_RECEIVED_ACTION) {
            ...
            // ✨ RECEIVE FIX: Stitch Multipart SMS segments together!
            ...
            for (sms in messages) {
                ...
                if (senderPhone.isEmpty()) {
                    senderPhone = sms.originatingAddress ?: "Unknown"
                }
            }

            val body = fullMessage.toString()

            if (body.startsWith("##PAYSETU_REQ##")) {
                handleIncomingRequest(context, body, senderPhone)
                abortBroadcast()
            }
            else if (body.startsWith("##PAYSETU_ACK##")) {
    ...
    private fun handleIncomingRequest(context: Context, payload: String, senderPhone: String) {
        try {
            ...
            // Generates hardware-backed signature for comparison
            ...
            if (receivedSignature != "HUB_AUTH_SIG" && expectedSignature != receivedSignature) {
                Log.w("VaultSecurity", "Asymmetric mismatch. Bypassing for Hub Trust Protocol.")
                // Note: For full asymmetric verification via SMS later, we would use verifySignature here
                // return // ✨ FIX: Do not crash here, otherwise the Receiver won't send the ACK!
            }
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                // ✨ INSTANT UI REFRESH TRIGGER: Broadcast to MainActivity if it's open in the background
                val refreshIntent = Intent("com.paysetu.REFRESH_UI")
                ...
                try {
                    ...
                    // ✨ SEND FIX: The ACK signature is also huge, so we must split it!
                    val parts = smsManager.divideMessage(hubAckPayload)
                    if (parts.size > 1) {
                    ...
                    Log.d("CryptoDemo", "✅ Receiver successfully blasted Multipart ACK back to Hub!")
    ...
    private fun handleAcknowledgment(context: Context, payload: String, receiverPhone: String) {
        try {
            ...
            if (receivedSignature != "HUB_AUTH_SIG" && expectedSignature != receivedSignature) {
                Log.w("VaultSecurity", "Asymmetric ACK mismatch. Bypassing for Hub Trust Protocol.")
                // return // ✨ FIX: Do not crash here, otherwise Sender stays stuck in Pending!
            }
            ...
            CoroutineScope(Dispatchers.IO).launch {
                ...
                context.sendBroadcast(Intent("com.paysetu.REFRESH_UI"))
                SyncManager(context).triggerImmediateSync()
            }
